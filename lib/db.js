...
export async function createTablet(input) {
  ...
  const tablet = {
    ...
    tabletsPerStrip: normalizePackInt(input.tabletsPerStrip),
    stripsPerPacket: normalizePackInt(input.stripsPerPacket),
    startDate: input.startDate || '',
    manufacturingDate: input.manufacturingDate || null,
    endDate: input.endDate || '',
    barcodeValue: input.barcodeValue ? String(input.barcodeValue).trim() : null,
...
export async function updateTablet(id, input) {
  ...
  tablets[idx] = {
    ...
    barcodeValue:
      input.barcodeValue === '' ? null : input.barcodeValue ?? current.barcodeValue ?? null,
    tabletsPerStrip:
      input.tabletsPerStrip === undefined
        ? current.tabletsPerStrip ?? null
        : normalizePackInt(input.tabletsPerStrip),
    stripsPerPacket:
      input.stripsPerPacket === undefined
        ? current.stripsPerPacket ?? null
        : normalizePackInt(input.stripsPerPacket),
  };
  await writeAll(tablets);
  return tablets[idx];
}
