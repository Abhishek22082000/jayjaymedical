...
const VALID_UNITS = new Set(['tablet', 'strip', 'packet']);
function normalizeUnit(v) {
  if (typeof v === 'string' && VALID_UNITS.has(v)) return v;
  return 'tablet';
}
...
export async function createTablet(input) {
  ...
  const tablet = {
    ...
    quantityUnit: normalizeUnit(input.quantityUnit),
  ...
  tablets[idx] = {
    ...
    quantityUnit:
      input.quantityUnit === undefined
        ? current.quantityUnit ?? 'tablet'
        : normalizeUnit(input.quantityUnit),
  };
  await writeAll(tablets);
  return tablets[idx];
}
